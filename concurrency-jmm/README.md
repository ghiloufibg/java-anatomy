...
|---|---|---|
| medium | `StoreBuffering` | `mvn -q -pl concurrency-jmm compile exec:exec -Dexec.mainClass=dev.sevenrungs.concurrency.StoreBuffering` |
| medium | `StoreBuffering` (volatile) | `... -Dexec.programArgs=volatile` |
| hard | `PhaseGate` | `mvn -q -pl concurrency-jmm compile exec:exec -Dexec.mainClass=dev.sevenrungs.concurrency.PhaseGate` |
| guru | `QuorumRead` | `mvn -q -pl concurrency-jmm compile exec:exec -Dexec.mainClass=dev.sevenrungs.concurrency.QuorumRead` |
| exercise | `PinDemo` | `mvn -q -pl concurrency-jmm compile exec:exec -Dexec.mainClass=dev.sevenrungs.concurrency.PinDemo` |
...
- **`concurrency-jmm-jcstress`** ([README](../concurrency-jmm-jcstress/README.md)) — the SB/MP/IRIW suite (one
  sentence per outcome, in each `@Outcome(desc = ...)`) and a stress test for the lock above.

## Tests

`mvn -q -pl concurrency-jmm test` runs the JUnit 5 suite. `StoreBufferingTest` only asserts the one thing
